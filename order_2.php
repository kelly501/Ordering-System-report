...
<?php
mysql_connect("localhost","abcabc","123") or die("could not connect:".mysql_error());
...
if ($do=='order'){
  for($i=0;$i<$count;$i++){
  	$sql[$i]="SELECT * FROM `$user` WHERE `item`='".$in[$i]."'";
  	$result=mysql_query($sql[$i]);
  	$row=mysql_num_rows($result);
  	if($row==0 && $nb[$i]!=0){
  			$sql[$i]="INSERT INTO `$user`(`item`, `number`, `price`) VALUES ('".$in[$i]."','".$nb[$i]."','".$ip[$i]."')";
...
<input style="width:120px;height:30px;margin:3px;background-color:pink;border-radius:10px;border:2px #FFB6C1 solid;" type="button" value="still buy" onclick="location.href='menu.php?p=1'">
<input style="width:120px;height:30px;margin:3px;background-color:pink;border-radius:10px;border:2px #FFB6C1 solid;" type="button" value="view purchases" onclick="location.href='order.php?p=1'">
</div>
</section>
