...
if ($del=='delete'){
	...
	for($i=0;$i<$count;$i++){
		if(isset($check[$i])){
		if($check[$i]=='check'){
			$sql[$i]="DELETE FROM `$user` WHERE `item` = '".$item[$i]."'";
...
<head>
<meta charset="UTF-8">
<tr>delete successful !</tr>
<br><br>
</head>
<input style='width:120px;height:30px;margin:3px; background-color:pink;border-radius:10px;border:2px #FFB6C1 solid;'type="button" value="menu" onclick="location.href='menu.php?p=1'">
